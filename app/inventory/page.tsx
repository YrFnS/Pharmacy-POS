    ...
    <div className="flex h-full flex-col bg-[#F9FAFB]">
      ...
      <header className="flex h-16 shrink-0 items-center justify-between border-b border-zinc-200/50 bg-white/80 px-8 backdrop-blur-xl">
        <div>
          ...
          <p className="text-xs font-bold uppercase tracking-widest text-zinc-500">Master Product List</p>
        </div>
        <div className="flex gap-3">
           <Button variant="outline" className="gap-2 rounded-xl text-zinc-700 border-zinc-300 hover:bg-zinc-50" onClick={handleExport}>
        ...
        <div className="mx-auto max-w-6xl space-y-6">
             ...
             <div className="rounded-2xl border border-zinc-200/50 bg-white p-5 shadow-sm">
               <div className="flex items-center gap-3">
                 ...
                 <div>
                   ...
                   <p className="text-2xl font-black text-zinc-900">{expiringCount}</p>
                 </div>
               </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between rounded-2xl border border-zinc-200/50 bg-white p-3 shadow-sm">
            <div className="relative w-80">
              ...
              <Input 
                className="h-10 rounded-xl border-none bg-zinc-50 ps-10 font-medium text-zinc-900 shadow-none focus-visible:ring-1 focus-visible:ring-zinc-900"
            ...
            <div className="flex gap-2 relative">
              ...
              <Button 
                variant="outline" 
                className={cn("gap-2 rounded-xl text-zinc-600", filterType !== 'All' && "bg-zinc-100 text-zinc-900 border-zinc-900/20")}
              ...
              {isFilterMenuOpen && (
                <div className="absolute right-0 top-12 z-20 w-48 rounded-xl border border-zinc-200 bg-white p-2 shadow-xl animate-in fade-in zoom-in-95 duration-200">
                  <button 
                    className={cn("w-full rounded-lg px-3 py-2 text-left text-sm font-bold transition-colors", filterType === 'All' ? "bg-zinc-100 text-zinc-900" : "text-zinc-500 hover:bg-zinc-50 hover:text-zinc-900")}
                    onClick={() => { setFilterType('All'); setIsFilterMenuOpen(false); }}
                  >
                    All Items
                  </button>
                  <button 
                    className={cn("w-full rounded-lg px-3 py-2 text-left text-sm font-bold transition-colors", filterType === 'Low Stock' ? "bg-zinc-100 text-zinc-900" : "text-zinc-500 hover:bg-zinc-50 hover:text-zinc-900")}
                    onClick={() => { setFilterType('Low Stock'); setIsFilterMenuOpen(false); }}
                  >
                    Low Stock
                  </button>
                  <button 
                    className={cn("w-full rounded-lg px-3 py-2 text-left text-sm font-bold transition-colors", filterType === 'Expiring' ? "bg-zinc-100 text-zinc-900" : "text-zinc-500 hover:bg-zinc-50 hover:text-zinc-900")}
          ...
          <div className="overflow-hidden rounded-2xl border border-zinc-200/50 bg-white shadow-sm flex flex-col h-[calc(100vh-320px)]">
            <div className="overflow-auto flex-1 relative">
              <table className="w-full text-left text-sm relative">
                <thead className="bg-zinc-50/90 backdrop-blur sticky top-0 z-10 shadow-sm text-zinc-600">
                  <tr>
                               ...
                               <table className="w-full text-sm">
                                         <thead>
                                            <tr className="border-b border-zinc-100">
                                               <th className="px-4 py-2 font-bold text-zinc-500 uppercase tracking-widest text-[9px] text-left">Batch No</th>
                                               <th className="px-4 py-2 font-bold text-zinc-500 uppercase tracking-widest text-[9px] text-left">Expiry</th>
                                               <th className="px-4 py-2 font-bold text-zinc-500 uppercase tracking-widest text-[9px] text-right">Qty</th>
                                               <th className="px-4 py-2 font-bold text-zinc-500 uppercase tracking-widest text-[9px] text-right">Price</th>
                                            </tr>
                                         </thead>
                                  <tbody>
                                         {p.batches.map(b => (
                                            <tr key={b.id} className="hover:bg-zinc-50/50">
                                               <td className="px-4 py-2 font-mono text-xs font-bold text-zinc-900">{b.batchNo}</td>
                                               <td className="px-4 py-2">
                                                  <span className={cn(
                                                    "font-semibold",
                                                    new Date(b.expiryDate) < new Date(new Date().setMonth(new Date().getMonth() + 6)) ? "text-amber-600" : "text-zinc-800"
                                                  )}>
                                                    {b.expiryDate}
                                                  </span>
                                               </td>
                                               <td className="px-4 py-2 text-right font-bold text-zinc-900">{b.quantity}</td>
                                               <td className="px-4 py-2 text-right font-bold text-zinc-900">{b.price.toLocaleString()} IQD</td>
                                            </tr>
                                         ))}
                                  </tbody>
                               </table>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-900/60 p-4 backdrop-blur-xl">
          <div className="w-full max-w-2xl rounded-3xl bg-white shadow-2xl flex flex-col max-h-[90vh]">
            ...
            <div className="flex-1 overflow-y-auto p-8 bg-zinc-50/50">
              <div className="grid grid-cols-2 gap-6">
                <div className="col-span-2">
                  <label className="mb-2 block text-xs font-bold uppercase tracking-widest text-zinc-600">Barcode</label>
                  <Input 
                    placeholder="Scan or enter barcode" 
                    className="h-12 rounded-xl bg-white text-zinc-900 focus-visible:ring-2 focus-visible:ring-zinc-900" 
                    value={newProduct.barcode}
                    onChange={e => setNewProduct({...newProduct, barcode: e.target.value})}
                  />
                </div>
                <div>
                  <label className="mb-2 block text-xs font-bold uppercase tracking-widest text-zinc-600">Brand Name</label>
                  <Input 
                ...
                <div>
                  <label className="mb-2 block text-xs font-bold uppercase tracking-widest text-zinc-600">Generic Name</label>
                  <Input 
                ...
                <div>
                  <label className="mb-2 block text-xs font-bold uppercase tracking-widest text-zinc-600">Category</label>
                  <select 
                ...
                <div className="col-span-2 pt-4 border-t border-zinc-100">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="mb-2 block text-[10px] font-bold uppercase tracking-widest text-zinc-600">Batch Number</label>
                      <Input 
                    ...
                    <div>
                      <label className="mb-2 block text-[10px] font-bold uppercase tracking-widest text-zinc-600">Sale Price (IQD)</label>
                      <Input 
                        type="number"
                        placeholder="0"
                        className="text-zinc-900"
            ...
            <div className="border-t border-zinc-100 bg-white p-6 flex justify-end gap-3">
               <Button variant="outline" className="h-12 font-bold px-8 text-zinc-600 border-zinc-200 hover:bg-zinc-50" onClick={() => setIsAddModalOpen(false)}>Cancel</Button>
      ...
      {editingProduct && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-900/60 p-4 backdrop-blur-xl">
          <div className="w-full max-w-2xl rounded-3xl bg-white shadow-2xl flex flex-col">
            <div className="flex items-center justify-between border-b border-zinc-100 px-8 py-6">
              <div className="flex items-center gap-3">
                 <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-amber-50 text-amber-600">
                    <Edit2 className="h-5 w-5" />
                 </div>
                 <h2 className="text-xl font-bold tracking-tight text-zinc-900">Edit Product</h2>
              </div>
              <button 
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="col-span-2">
                  ...
                  <Input 
                    ...
                    className="text-zinc-900 h-12 rounded-xl"
            ...
            <div className="border-t border-zinc-100 p-6 flex justify-end gap-3 bg-zinc-50/30 rounded-b-3xl">
               <Button variant="outline" className="h-12 font-bold px-8 text-zinc-600 border-zinc-200 hover:bg-zinc-50" onClick={() => setEditingProduct(null)}>Cancel</Button>
               <Button className="h-12 bg-zinc-900 shadow-lg font-bold px-8" onClick={handleUpdateProduct}>Update Changes</Button>
            </div>
          </div>
        </div>
      )}
